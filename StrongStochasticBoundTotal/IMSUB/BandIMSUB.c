/*  IMSUB with an internal band format,  Compute upper bound
 needs .Rxx matrix and .sz size files
 version long double, Result stored in Rii form. 
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define Gamma 0.1
#define epsNull 0.00000001

void  usage()
{
	printf("usage : BandRowIMSUB -f filename  \n");
...
char *argv[];
{
	...
	minr=minl;
    printf("init line 0 \n");
	for (j = minr; j <= maxr; j++) {printf("%d %d r %Lf %d \n", minr, maxr, r[j],j); } 
	...
	for (i = 1; i < NSommets ; i++) {
		...
		maxp=maxl;
		minp=minl;
        for (j = minp; j <= maxp; j++) {printf("%d %d, p, %Lf %d \n", minp, maxp , p[j],j); } 
	...
	fprintf(pf1,"%d\n", NEt);
	fclose(pf1);
	printf("Done BandRowIMSUB Upper Bound\n");
	exit(0);
}

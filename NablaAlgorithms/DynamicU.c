/*
 * JMF Dynamic Nabla Full Matrix borne sup
 */
...
usage()
{
	printf("usage : DynamicU -f filename \n");
...
char           *argv[];
{
    ...
    if (!(dinf = (double * )malloc(NRow*sizeof(double)))) 
        ProblemeMemoire();
    if (!(dsup = (double * )malloc(NRow*sizeof(double)))) 
        ProblemeMemoire();
	...
	for (j = 0; j < NRow; j++) {dinf[j]=0.1 ;dsup[j]=0.0;}
 	for (i = 0; i < NRow; i++) {
		/*
		 * Lecture de la ligne
		 */
